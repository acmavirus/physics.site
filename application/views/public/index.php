...
<body>
    ...
    <script>
        (function() {
            ...
            function parseWords(raw) {
                if (Array.isArray(raw)) return raw.map(function(w) {
                    ...
                    if (w && typeof w === 'object') {
                        return {
                            text: w.text || w.word || '',
                            size: w.size || w.weight || 1 + Math.random(),
                            slug: w.slug
                        }
                    }
                    return {
            ...
            var phpWords = <?php echo isset($words) ? json_encode($words, JSON_UNESCAPED_UNICODE) : 'null'; ?>;
            var phpText = <?php echo isset($text) ? json_encode($text, JSON_UNESCAPED_UNICODE) : 'null'; ?>;
            var categoryBase = <?php echo json_encode(isset($category_base) ? $category_base : '/danh-muc/'); ?>;
            var words = phpWords ? parseWords(phpWords) : buildWordsFromText(phpText);
            if (!words) words = parseWords(null);
            var baseSizes = words.map(function(d) {
                return d.size || 1
            });
            ...
            function render() {
                ...
                d3.layout.cloud().size([width, height]).words(words).padding(1).rotate(function() {
                    return 0
                }).font('Noto Sans,STIX Two Text,system-ui,-apple-system,Segoe UI,Roboto,Helvetica Neue,Arial').fontSize(function(d) {
                    return d.size
                }).spiral('rectangular').on('end', function(layoutWords) {
                    ...
                    g.selectAll('text').data(layoutWords).enter().append('text').attr('text-anchor', 'middle').style('font-size', function(d) {
                        return d.size + 'px'
                    }).style('font-family', 'Noto Sans,STIX Two Text,system-ui,-apple-system,Segoe UI,Roboto,Helvetica Neue,Arial').style('font-weight', '800').style('fill', function(d, i) {
                        return '#000'
                    }).style('cursor', 'pointer').style('paint-order', 'stroke fill').attr('transform', function(d) {
                        return t(d)
                    }).text(function(d) {
                        return d.text
                    }).on('mouseover', function(event, d) {
                        if (activeNode && activeNode !== this) {
                            d3.select(activeNode).interrupt().transition().duration(80)
                                .attr('transform', t(activeDatum));
                        }
                        activeNode = this;
                        activeDatum = d;
                        ...
                        sel.raise();
                        sel.interrupt();
                        sel.style('fill', 'rgba(247, 139, 139, 1)').style('stroke-width', '2px');
                        sel.transition().duration(220).ease(d3.easeCubicOut)
                            .attr('transform', t(d) + ' scale(1.12)')
                    }).on('mouseout', function(event, d) {
                        if (this !== activeNode) return;
                        var sel = d3.select(this);
                        sel.interrupt();
                        sel.style('fill', '#000').style('stroke-width', '0px');
                        sel.transition().duration(200).ease(d3.easeCubicOut)
                            .attr('transform', t(d));
                        activeNode = null;
                        activeDatum = null;
                    }).on('click', function(event, d) {
                        var slug = d.slug || slugify(d.text);
                        window.location.href = categoryBase + slug;
                    })
                }).start()
            }
            render();
